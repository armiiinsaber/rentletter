...
const junction = (extra = {}) => ({ decisionStatus: 'none', withdrawnAt: null, decisionChangedAt: null, docRequest: null, ...extra });
...
test('verified: a report with income matched', () => { assert.deepEqual(applicantState({ junction: junction(), verification: [report()] }), { state: 'verified', since: '2026-08-12T15:00:00Z' }); });
...
test('priority: set aside beats verified, mismatch beats verified, a report beats a pending request', () => {
  assert.equal(applicantState({ junction: junction({ decisionStatus: 'reject' }), verification: report() }).state, 'set_aside');
  assert.equal(applicantState({ junction: junction(), verification: report({ nameMatch: 'mismatch' }) }).state, 'mismatch');
  assert.equal(applicantState({ junction: junction({ docRequest: { status: 'requested', requestedAt: '2026-08-30T10:00:00Z' } }), verification: report() }).state, 'verified');
