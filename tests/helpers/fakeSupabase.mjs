...
export function fakeSupabase(tables, { absentColumns = [] } = {}) {
  ...
  const from = (table) => {
    ...
    const run = async () => {
      ...
      if ((q.op === 'update' || q.op === 'insert' || q.op === 'upsert') && q.payload) { const bad = Object.keys(Array.isArray(q.payload) ? q.payload[0] || {} : q.payload).find((c) => absentColumns.includes(c)); if (bad) return { data: null, error: { code: '42703', message: `column ${table}.${bad} does not exist` } }; }
