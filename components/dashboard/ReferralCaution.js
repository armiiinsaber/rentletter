...
export default function ReferralCaution({ meta, compact = false }) {
  ...
  return (
    <div style={{ marginTop: compact ? 8 : 12, padding: compact ? '8px 12px' : '10px 14px', background: C.paperDeep, border: `1px solid ${C.rule}`, borderLeft: `3px solid ${C.ink}`, borderRadius: R.ctrl, fontSize: 12.5, color: C.inkSoft, lineHeight: 1.55 }}>
      <div style={{ color: C.ink, fontWeight: 700 }}>Referred by {meta.fromName || 'another realtor'}{meta.fromBrokerage ? ` · ${meta.fromBrokerage}` : ''}{meta.approvedAt ? ` · applicant approved ${dateLong(meta.approvedAt)}` : ''}</div>
      {meta.note && <div style={{ marginTop: 3, fontStyle: 'italic' }}>“{meta.note}”</div>}
      {v ? (
        <div style={{ marginTop: 6, padding: '8px 10px', background: C.amberTint, borderLeft: `3px solid ${C.amber}`, borderRadius: R.ctrl, color: C.ink }}>
          ...
          {' '}<strong>This is not a verification for your listing — re-request documents if it matters.</strong> No documents were transferred.
        </div>
      ) : (
        <div style={{ marginTop: 4 }}>No document check was shared. Request documents as you would for any applicant.</div>
      )}
    </div>
  );
}
