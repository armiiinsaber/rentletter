...
import { normalizeEmployer, employerStatus } from './employerName.js';
...
function sameValue(fact, a, b) {
  ...
  if (fact === 'employer') return normalizeEmployer(a) === normalizeEmployer(b); // lib/employerName.js
  if (fact === 'title') return orgTokens(a).join(' ') === orgTokens(b).join(' ');
...
// Job title: the same after normalization is a match; a shared significant word is close; else a
// mismatch. Nothing resolved is not_found. (Employers compare through lib/employerName.js.)
...
// Employer: lib/employerName.js normalizeEmployer, then equal is match, one containing the other is close.
export const employerTextStatus = (stated, found) => (present(stated) && present(found) ? employerStatus(stated, found) : 'not_found');
...
export function employerComparison(documents, stated, { now = new Date() } = {}) {
  ...
  const row = { field: 'Employer', stated: present(stated.statedEmployer) ? String(stated.statedEmployer) : null, found: r.value, status: employerTextStatus(stated.statedEmployer, r.value), source: r.source ? r.source.kind : null, alsoSeen: r.alsoSeen.map((e) => e.line) };
