...
export function inviteFromRow(row, profile) {
  ...
  return {
    ...
    listingName: String(displayLabel(row, 'Listing')).slice(0, 80),
    unit: {
...
export async function loadInviteListing(admin, token) {
  ...
  // The state is what listingOpen reads; status rides along for the fallback. Where the state
  // column is absent (before db/002, or after db/999) the same row is read without it.
  const q = (cols) => admin.from('listings').select(cols).eq('invite_token', String(token)).maybeSingle();
  let found = await q(`${LISTING_COLS}, state`);
  if (found.error) found = await q(LISTING_COLS);
