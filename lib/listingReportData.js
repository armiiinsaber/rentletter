...
export async function loadReportContext(serverClient, adminClient, listingId, userId) {
  ...
  try {
    const { data: extras } = await adminClient
      .from('listing_applicants').select('id, application_id, doc_verifications').eq('listing_id', listing.id);
    ...
    console.log('[verif-trace][report] listing=%s extras=%j', listing.id,
    ...
    for (const a of applicants) {
      ...
      // STRICT per-row attribution: an applicant is verified ONLY from the junction row that
      // matches BOTH its own linkId AND its own application_id — never another row's value.
      let dv = null;
      if (e) {
        if (String(e.application_id) === String(ownApp)) dv = e.doc_verifications;
        else console.warn('[verif-trace][report] ATTRIBUTION MISMATCH linkId=%s applicantApp=%s rowApp=%s (ignoring)', a.linkId, ownApp, e.application_id);
      }
      a.verification = landlordVerification(dv);
      // [temp diagnostic] per-applicant attribution result.
      console.log('[verif-trace][report] linkId=%s app_id=%s ownDocVerif=%s verified=%s',
        a.linkId, ownApp, dv != null, a.verification.verified);
    }
  } catch (e) {
    console.error('[verif-trace][report] read failed:', e?.message || e);
    for (const a of applicants) a.verification = landlordVerification(null);
  }
