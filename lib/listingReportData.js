...
import { compareFit } from './fitScore';
import { authorizeApplicant } from './applicantAnalysis';
import { activeReport } from './docVerifications';
import { isWithdrawn, isSetAside } from './listingApplicantsVocabulary';


// Fit first on the exact score (lib/fitScore.js compareFit); an applicant without a Fit (no rent
// share) sorts after every scored one; a tie goes to the earlier applicant.
...
// documents (held for the realtor only, never on a report), any 'mismatch'/'close' comparison, and any cross-reference
// 'discrepancy'. Never touches OHRC-protected data.
export function landlordVerification(docVerifications) {
  // Normalize the old bare-array OR new {active,archived} shape → the current (active) report.
  const latest = activeReport(docVerifications);
  ...
  // The employer status is recomputed from the stored row at read time (lib/employerName.js).
  const employerRow = comparisons.find((c) => c && /employer/i.test(String(c.field || '')));
  const employerM = employerRowStatus(employerRow) === 'match' ? employerRow : null;
...
export function verificationText(v) {
  ...
  if (v.incomeVerified) parts.push(`income matched documents${v.incomeFigure ? ` (${v.incomeFigure})` : ''}`);
...
export function verificationConfirmText({ realtorName, brokerage, phone, unitName, applicantName, verification }) {
  ...
  if (v.verified) {
    ...
    if (v.incomeVerified) out.push(leader('Income', `matched documents${v.incomeFigure ? ` (${v.incomeFigure})` : ''}`));
...
// Returns { active, setAside }, each best fit first; withdrawn excluded (withdrawn_at rule).
export function rankApplicants(list) {
  const active = [];
  const setAside = [];
  for (const a of list || []) {
    if (isWithdrawn(a)) continue;
    (isSetAside(a) ? setAside : active).push(a);
  }
  active.sort(compareFit);
  setAside.sort(compareFit);
...
export async function loadReportContext(serverClient, adminClient, listingId, userId) {
  ...
  // The document report is attached so Fit (which reads it) is computed for the report's
  // ranking. The GROUP shortlist still prints no per applicant verification detail; the
  // single applicant confirmation is a separate path (loadApplicantVerification).
