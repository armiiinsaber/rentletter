...
export function landlordVerification(docVerifications) {
  ...
  if (!latest || docs.length === 0) return { verified: false };
  ...
  // Credit facts come straight from a recognized (non-flagged) credit-report document.
  const creditDoc = docs.find((d) => /credit\s*report/i.test(String(d.documentType || '')) && d.unrecognized !== true && d.extracted);
...
export async function loadReportContext(serverClient, adminClient, listingId, userId) {
  ...
  try {
    const { data: extras } = await adminClient
      .from('listing_applicants').select('id, doc_verifications').eq('listing_id', listing.id);
    const m = new Map(Array.isArray(extras) ? extras.map((e) => [e.id, e.doc_verifications]) : []);
    for (const a of applicants) a.verification = landlordVerification(m.get(a.linkId));
  } catch (e) {
