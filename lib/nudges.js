...
import { SET_SENTENCE_LOWER } from './documentSet.js';
import { listingOpen, listingStatus } from './listingState.js';
...
export function selectNudge(ctx, now = new Date()) {
  ...
  const standing = applicantStanding(junction);
  if (standing.withdrawn) return { drop: 'withdrawn' };
  if (standing.setAside) return { drop: 'set aside' };
  if (!listing) return { drop: 'listing gone' };
  if (!listingOpen(listing)) return { drop: `listing ${listingStatus(listing)}` };
...
export function nudgeEmail({ nudge, listingName, realtorName, applicantName, uploadUrl }) {
  ...
  const paras = [
    `Your application for ${unit} is in. It is waiting on your documents: ${SET_SENTENCE_LOWER}.`,
...
export async function runNudges(deps, { now = new Date(), log = console.log } = {}) {
  ...
  for (let i = 0; i < linkIds.length; i++) {
    ...
    const mail = nudgeEmail({ nudge: decision.nudge, listingName: displayLabel(listing), realtorName, applicantName: application.full_name, uploadUrl: kv.uploadUrl(pointer.token) });
    try {
      ...
      // The upload link in this email must outlive the email: the request record gets 7 days from now too.
      if (kv.expire && kv.reqKey && pointer.token) await kv.expire(kv.reqKey(pointer.token), kv.ttl);
