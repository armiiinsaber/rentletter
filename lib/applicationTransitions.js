...
import {
  ...
  applicationStateOf, listingStateOf,
...
export async function transitionApplication(admin, { junction, listing = null, to, patch = {}, actor = null, actorType = ACTOR_TYPE.REALTOR, reason = null }) {
  ...
  if (changed) assertTransition(from, to);
  ...
  if (stored) await writeAudit(admin, [auditRow({ id: junction.id, from, to }, { actor, actorType, reason })]);
