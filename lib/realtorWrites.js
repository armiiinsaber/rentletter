...
import { APPLICATION_STATE, ACTOR_TYPE, LEGACY_LISTING_STATUS, LISTING_DELETED, LISTING_STATE, RECONSIDER_REASONS, RECONSIDER_UNDONE, TransitionError, applicationStateOf, isInPlay, isTransitionError } from './application-state.js';
...
export async function decideApplicant(deps, body) {
  ...
  // The finalist mark is the move between in play and shortlisted (lib/application-state.js).
  // Set aside is the realtor's working sort and can be undone, so it never moves the state, and
  // a mark on an application that is no longer in play moves nothing either.
  const from = applicationStateOf(junction, listing);
  let to = null;
  // A reconsidered applicant comes back through the same mark: shortlisted is their only way forward.
  if (priority != null && (isInPlay(from) || from === APPLICATION_STATE.RECONSIDERED)) {
    const aside = (status != null ? status : junction.decision_status) === DECISION_STATUS.REJECT;
    if (priority === DECISION_PRIORITY.TOP && !aside) to = APPLICATION_STATE.SHORTLISTED;
    else if (from === APPLICATION_STATE.SHORTLISTED) to = APPLICATION_STATE.SUBMITTED;
  }
  let moved;
...
export async function withdrawApplicant(deps, body) {
  ...
  const from = applicationStateOf(junction, listing);
  let to = null;
  if (withdrawn) to = APPLICATION_STATE.WITHDRAWN_BY_APPLICANT;
  else if (from === APPLICATION_STATE.WITHDRAWN_BY_APPLICANT) {
    ...
    to = applicationStateOf({ ...junction, state: null, withdrawn_at: null }, null);
  }
  let moved;
...
export async function reconsiderApplicant(deps, body) {
  ...
  let moved;
  try { moved = await transitionApplication(admin, { junction: own.junction, listing, to, actor: userId, actorType: ACTOR_TYPE.REALTOR, reason: undo ? RECONSIDER_UNDONE : reason }); }
  catch (e) { if (isTransitionError(e)) return refusal(e); throw e; }
