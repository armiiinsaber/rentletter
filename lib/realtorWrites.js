...
import { APPLICATION_STATE, ACTOR_TYPE, LEGACY_LISTING_STATUS, LISTING_DELETED, LISTING_STATE, applicationStateOf, isInPlay, isTransitionError } from './application-state.js';
import { transitionApplication, transitionListing, refusal, listingRefusal } from './applicationTransitions.js';
...
export async function decideApplicant(deps, body) {
  ...
  let to = null;
  if (priority != null && isInPlay(from)) {
...
export async function withdrawApplicant(deps, body) {
  ...
  else if (from === APPLICATION_STATE.WITHDRAWN_BY_APPLICANT) to = applicationStateOf({ ...junction, state: null, withdrawn_at: null }, null);
  let moved;
