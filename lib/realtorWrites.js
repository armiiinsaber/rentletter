...
// TODO: the applicant hears about this through a re invite, never a silent change. The message
// goes in lib/reconsiderInvite.js (not built yet) and is sent from here once the move is stored.
export async function reconsiderApplicant(deps, body) {
  ...
  invalidate?.(userId);
  return { status: 200, body: { ok: true, linkId: own.junction.id, state: moved.to } };
}
