...
// the demo fixture (lib/demoFixture.js) produce the SAME curve.
//
// The scorecard shape (scorecard-v2): three 0 to 5 sub-scores (incomeStability, rentAffordability,
// rentalHistory) + an equal-weight `overall`. The numbers are continuous instead of hard bands,
// so small differences move the score only a little and the other factors break ties.
//
// OHRC / BC Code: only ever screenable factors: income, rent-to-income, employment tenure,
// rental history, references. Never age, family status, income source, and never anything
// parsed out of a free text answer.
...
// Equal-weight overall of the THREE scorecard-v2 components. Any other key on `scores` (a stored
// five component object from before v2) is ignored, so old rows average correctly to three.
export function overallScore(scores) {
  const { incomeStability, rentAffordability, rentalHistory } = scores || {};
  const n = (v) => (Number.isFinite(Number(v)) ? Number(v) : 0);
  return round1((n(incomeStability) + n(rentAffordability) + n(rentalHistory)) / 3);
}
