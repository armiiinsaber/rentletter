...
// An "a · b · c" line: the separator is tied to the item that follows it, so a wrap falls between
// items and the dot never lands alone at the start or the end of a line.
export const dots = (text) => String(text ?? '').replace(/ ([·|]) /g, (m, sep) => ` ${sep}${NBSP}`);
