...
import { ageConfirmedOf } from './applicationMap.js';
...
export function rowToForm(row) {
  ...
  const f = { ...EMPTY_FORM };
  f.email = s(a.email);
  f.fullName = s(a.full_name); f.phone = s(a.phone);
  f.ageConfirmed = ageConfirmedOf(a); // the age_confirmed column, the scorecard key only while the column is absent
  ...
  // Name, employer, job title and income only: age and relationship are never read back.
  if (co) { f.hasCoApplicant = true; f.coApplicantName = s(co.name); f.coApplicantJobTitle = s(co.jobTitle); f.coApplicantEmployer = s(co.employer); f.coApplicantIncome = s(co.annualIncome ?? co.annual_income); }
