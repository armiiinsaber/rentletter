...
export async function fetchListingApplicants(admin, listingId) {
  ...
  // Optional columns added by later migrations, tried richest-first and dropped one at a time
  // when the DB doesn't have them yet: doc_verifications (so de-dup can KEEP the junction row
  // holding the analysis) and reviewed_at (db/reviewed-at.sql — per-applicant reviewed state).
  const q = (extra) => admin.from('listing_applicants')
    .select(`${COLS}${extra ? `, ${extra}` : ''}, application:applications(*)`)
    .eq('listing_id', listingId).order('created_at', { ascending: true });
  let sel = await q('doc_verifications, reviewed_at');
  let hasReviewedAt = true;
  if (sel.error) { sel = await q('doc_verifications'); hasReviewedAt = false; }
  if (sel.error) { sel = await q('reviewed_at'); hasReviewedAt = true; }
  if (sel.error) { sel = await q(''); hasReviewedAt = false; }
