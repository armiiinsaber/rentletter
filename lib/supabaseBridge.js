...
export async function fetchListingApplicants(admin, listingId) {
  ...
  // (db/listing-applicants-vocabulary.sql), confirmations and last_sent_at (db/screening.sql). Tried all at once; if that fails, each column is
  // probed on its own and only the ones the DB has are kept.
  const OPTIONAL = ['doc_verifications', 'reviewed_at', 'withdrawn_at', 'confirmations', 'last_sent_at'];
...
export async function attachDocVerifications(admin, listingId, applicants, ctxLabel = 'read') {
  ...
  try {
    ...
    await attachStoredDocuments(admin, list);
  } catch (e) {
    console.warn('[verif-trace][%s] doc_verifications read skipped: %s', ctxLabel, e?.message || e);
    for (const a of list) { if (a.docVerifications === undefined) a.docVerifications = []; if (a.docArchived === undefined) a.docArchived = []; if (a.aiInsight === undefined) a.aiInsight = null; if (a.storedDocuments === undefined) a.storedDocuments = null; attachFit(a, null); }
  }
  return list;
}
