...
async function attachSideData(admin, list) {
  ...
  await Promise.all([attachDocRequests(list), attachStoredDocuments(admin, list), attachLandlordAnswers(admin, [...new Set(list.map((a) => a.listingId).filter(Boolean))], list)]);
}
