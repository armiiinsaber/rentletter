...
import { attachLandlordAnswers } from './reportSnapshotStore';
...
import { attachReferenceResponses } from './referenceStore.js';
import { ACTOR_TYPE, carriedColumns, standingWeight } from './application-state.js';
import { startingState, recordStart, stateColumnAbsent } from './applicationTransitions.js';
...
// Returns { created }: false when the pair already had a row (a repeat mirror call), so the
// caller records applicant_applied once per application.
export async function linkApplicantToListing(admin, listingId, applicationId, addedVia) {
  const { data: existing } = await admin.from('listing_applicants').select('id').eq('listing_id', listingId).eq('application_id', applicationId).maybeSingle();
  if (existing?.id) return { created: false, linkId: existing.id };
  // A new application on a listing starts in submitted (lib/application-state.js): asserted like
  // any other move, written with the row, and audited once. Before db/002 has run the column is
  // absent and the row is written without it.
  const state = startingState();
  const row = { listing_id: listingId, application_id: applicationId, added_via: addedVia, decision_status: DECISION_STATUS.NONE };
  const write = (r) => admin.from('listing_applicants').upsert(r, { onConflict: 'listing_id,application_id', ignoreDuplicates: true });
  let { error } = await write({ ...row, state });
  let stored = !error;
  if (error && stateColumnAbsent(error)) { stored = false; ({ error } = await write(row)); }
  if (error) throw error;
  if (stored) {
    try {
      const { data: made } = await admin.from('listing_applicants').select('id').eq('listing_id', listingId).eq('application_id', applicationId).maybeSingle();
      if (made?.id) await recordStart(admin, { linkId: made.id, to: state, actor: applicationId, actorType: ACTOR_TYPE.APPLICANT, reason: `added_via_${addedVia || 'invite'}` });
    } catch (e) { console.warn('[supabaseBridge] starting state not audited:', e?.message || e); }
  }
  return { created: true, linkId: null };
}

// Read the applicants for MANY listings in ONE junction select (application body embedded,
// doc_verifications in the same row), grouped by listing in memory. The caller
...
//
// Optional columns added by later migrations: doc_verifications (db/doc-intel),
...
// docArchived and fit (attachDocVerifications is the pure mapping step over the rows
...
// state (db/002-application-state-tables.sql) is read first by every screen; the old columns ride along for the fallback.
const OPTIONAL_JUNCTION_COLS = ['doc_verifications', 'reviewed_at', 'withdrawn_at', 'confirmations', 'last_sent_at', 'state'];
export const JUNCTION_SELECT = (present) => `${JUNCTION_COLS}${present.length ? `, ${present.join(', ')}` : ''}, application:applications(*)`;

export async function fetchApplicantsForListings(admin, listingRows, { sideData = true, references = false } = {}) {
  ...
  const byKey = new Map();
  const score = (r) => (hasAnyDocV(r.doc_verifications) ? 2 : 0) + standingWeight(r);
  for (const r of rows) {
  ...
  for (const row of deduped) {
    ...
    Object.defineProperty(applicant, 'docRow', { value: { id: row.id, application_id: row.application_id, doc_verifications: row.doc_verifications }, enumerable: false });
    (byListing[row.listing_id] = byListing[row.listing_id] || []).push(applicant);
  }
  // Two applications from one person on one listing (lib/duplicates.js): the later carries duplicateOf.
  for (const id of Object.keys(byListing)) markDuplicates(byListing[id]);
  const all = Object.values(byListing).flat();
  mapDocVerifications(all);
  if (sideData) await attachSideData(admin, all, { references });
  return byListing;
}
...
// Attach the realtor-side doc_verifications to each applicant using STRICT two-key
...
export async function attachDocVerifications(admin, listingId, applicants) {
  ...
  await attachSideData(admin, list, { references: true }); // the listing page: the checklist shows the reference answers
  return list;
}

function mapDocVerifications(list) {
  for (const a of list) {
    if (a.docVerifications !== undefined) continue; // already mapped
...
async function attachSideData(admin, list, { references = false } = {}) {
  // The landlord's answer on the latest report snapshot rides on the applicant (lib/reportSnapshotStore.js).
  // The previous landlord's reference answers (lib/referenceStore.js) are read for the listing
  // page only; the dashboard load stays inside its query budget and never shows them.
  await Promise.all([attachDocRequests(list), attachStoredDocuments(admin, list), attachLandlordAnswers(admin, [...new Set(list.map((a) => a.listingId).filter(Boolean))], list), references ? attachReferenceResponses(admin, list) : Promise.resolve()]);
}
