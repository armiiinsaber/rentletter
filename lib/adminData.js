...
const isFounderRow = (p) => !!(p.is_founder || p.subscription_status === 'founder');
...
export async function renumberFounders(admin, { deletedIds = [], at = new Date().toISOString() } = {}) {
  ...
  const { data, error } = await admin.from('profiles').select('id, signup_number, is_founder, subscription_status, created_at, signup_number_history');
  if (error) {
    if (!/signup_number_history/.test(error.message || '')) { out.errors.push(error.message); return out; }
    out.historyColumnMissing = true;
    const r2 = await admin.from('profiles').select('id, signup_number, is_founder, subscription_status, created_at');
...
export async function deleteRealtors(ids, { deleteOrphanApplications }) {
  ...
  const { data: numbered } = await admin.from('profiles').select('id, signup_number, is_founder, subscription_status').in('id', ids);
