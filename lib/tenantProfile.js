...
export const EMPTY_FORM = {
  ...
  fullName: '', age: '', dateOfBirth: '', phone: '', ageConfirmed: false, // age and dateOfBirth are form only, never stored
...
export function formFromApplication(app) {
  ...
  const m = a.move || {}, h = a.household || {}, l = a.lifestyle || {}, co = a.coApplicant || null;
  ...
  f.fullName = s(t.fullName); f.phone = s(t.phone);
  f.ageConfirmed = t.ageConfirmed === true; // the date of birth is never read back
  ...
  f.moveInDate = isoFromStoredDate(m.moveInDate);
  ...
  if (co) {
    ...
    f.coApplicantName = s(co.name); // age and relationship are never read back
...
// apartment, coverLetter. Recomputes: monthlyIncome, rentToIncomeRatio. scorecard is written null.
...
export function buildApplicationFromForm(existing, rawForm) {
  ...
  const estimatedRent = int(ap.estimatedRent) || null; // the rent the application arrived under (pages/api/generate.js), never parsed from text
  ...
  // The scorecard is no longer computed (Fit is the number); the key is written null.
  void householdAnnualIncome; void householdRentToIncomeRatio;
  const scorecard = null;

  return {
    ...existing,
    email,
    tenant: { fullName, ageConfirmed: existing?.tenant?.ageConfirmed === true || f.ageConfirmed === true, phone: str(f.phone, 40) },
    employment: {
    ...
    move: { moveInDate, reasonForMoving: null }, // reason removed from every form, written null
    household: {
      ...
      evParkingNeeded: null, // removed from every form, written null
    },
    coApplicant: hasCo ? {
      name: str(f.coApplicantName, 120), age: null, relationship: null, // never written: a protected ground and a family status proxy
    ...
    vehicle: null, // removed from every form, written null
    references,
    disclosures: null, // removed from every form, written null
    scorecard,
...
export function publicProfile(app) {
  ...
  return {
    ...
    vehicle: null, // no longer collected or shown
    references: Array.isArray(a.references) ? a.references : [],
    disclosures: null, // no longer collected or shown
  };
}
