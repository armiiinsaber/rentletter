...
export const PET_SIZE_LABELS = { small: 'small (under 25 lb)', medium: 'medium (25–60 lb)', large: 'large (60+ lb)' };
...
export function formFromApplication(app) {
  ...
  f.personality = s(l.personality); f.pets = s(l.pets); f.redFlags = s(a.disclosures);
...
export function buildApplicationFromForm(existing, rawForm) {
  ...
  return {
    ...
    employment: { jobTitle, employer, yearsAtJob: str(f.yearsAtJob, 10), annualIncome: annualIncomeNum, monthlyIncome },
    ...
    lifestyle: { personality: str(f.personality, 500), pets: str(f.hasPets ? serializePets(f) : '', 300) },
