...
1. **For tenants:** Submit one standardized rental application, free. You get a unique application number (RL-2026-XXXX-XXXX) you can share with realtors and landlords, and a tenant resume built from your answers. No cover letter is written.

2. **For realtors:** A dashboard where every application through a listing's invite link arrives ranked against the landlord's stated criteria. The realtor verifies, sets aside for screenable reasons, and sends the landlord a frozen report page where the landlord answers per applicant. Realtors sign in with their email.

## Pricing (for tenants)

- **Application:** free. There is nothing for a tenant to buy. Rentletter no longer sells cover letters or passes.
- **Editing:** a tenant updates their application any time from /my-application with the owner token from their confirmation email.
- **Realtor plan:** the first 50 realtors are founders (free); later signups get a 7 day trial and then a subscription. Landlords never sign in and never pay: they receive a private report page.
...
2. Submits. Nothing is paid.
3. Receives an email with: the application number (RL-2026-XXXX-XXXX), the owner token, "Manage your application" link.
4. Tenant shares their application number with landlords by email or text.
5. The realtor who owns the listing sees the application ranked on their dashboard; a landlord sees it only on the report page the realtor sends.

## How the landlord flow works

1. The realtor sends the landlord the report by email from the listing's "Present to landlord" section.
2. The email carries one button to a private page at rentletter.ca/r/{token}; no login, no account.
3. The page shows the applicants ranked best fit first with the Fit number, one fact sentence each, the realtor's confirmations, and four numbers: income, rent share, years at the job, references. Screenable facts only, no free text.
4. The landlord taps "I'd like to meet them" or "Not for me" per applicant; the realtor sees the answer on the applicant's card, in the Next list and on the timeline.
5. The page and the attached PDF come from the same frozen snapshot, so a later edit never changes what the landlord saw. The page expires after 90 days.
...
A unique application identifier in the format RL-2026-XXXX-XXXX. A realtor adds an applicant to a listing by this number when the application did not arrive through the listing's invite link.
...
- Landlords have no account: the report page is private to its link and shows screenable facts only.
...
- **"Can I edit my application after submitting?"** Yes, free, from /my-application with the owner token in your confirmation email. Every realtor who has your number sees the update.
...
- **"Refund policy?"** Nothing is charged to tenants, so there is nothing to refund.

## Common landlord questions

- **"Does my landlord need an account?"** No. The report page opens from the link in the email, with no login.
- **"How do I get tenants to apply via Rentletter?"** Share the listing's invite link, the short link or the QR from the Post kit.
...
- **"What if I'm not sure how to decide?"** Set aside records a screenable reason on the timeline; Rentletter never suggests who to pick.
- **"Can I use this on my phone?"** Yes. Sign in once on laptop, click the email link on phone, and your listings and applicants are there.
...
- Realtor subscriptions via Stripe. Tenants are never charged.
- Email via Resend.
...
- "Set aside" de-prioritizes an applicant with a SCREENABLE reason you record (e.g. affordability, limited rental history). They stay in your list, sorted to the bottom, and never reach the landlord's report page; the reason on the timeline is your record.
...
- Under an applicant, "Analyze documents" reads uploaded supporting documents (pay stubs, employment letters, credit report, ID), then holds the originals for the realtor's review for 14 days or until the realtor deletes them (a "Documents held" list under the applicant, with View and Delete all). Only that realtor can view them; every view is logged.
...
## Sending the report to your landlord
- "Present to landlord" freezes a snapshot and emails the landlord one button to a private report page (rentletter.ca/r/{token}), with the PDF from the same snapshot attached; "Copy text for landlord" gives the same list as a paste ready message. The landlord answers per applicant on the page and you see the answer on the card and the timeline. The group report is RANKING-ONLY; document verification is the separate Stage-2 per-applicant confirmation above. Fit indicators reference only legally screenable criteria.
