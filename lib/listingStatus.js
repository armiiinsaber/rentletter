...
// listing, the consent token, the read and the flip for /keep/{token}, and the absent table probe.
...
// Read a consent row for the /keep/{token} page. READ ONLY: one select on pipeline_consents and
// one on profiles for the realtor's name; never an update. Returns { found, expired, answered,
// status, realtorName }.
export async function readConsent(admin, token, { now = new Date() } = {}) {
  ...
  if (row.profile_id) {
    const { data: prof } = await admin.from('profiles').select('full_name').eq('id', row.profile_id).maybeSingle();
    realtorName = prof?.full_name || null;
  }
  return { found: true, expired, answered: row.status !== 'pending', status: row.status, realtorName };
}

// Flip a consent row by token, from a tap on the /keep page (POST /api/pipeline/answer), never
// from a page load. status: 'consented' | 'declined'. Refuses expired rows and rows already
// answered. Returns { ok, found, expired, answered, status }.
export async function flipConsent(admin, token, status, { now = new Date() } = {}) {
  ...
  const expired = !!(row.expires_at && new Date(row.expires_at).getTime() < new Date(now).getTime());
  if (expired) return { ok: false, found: true, expired: true, answered: false, status: row.status };
  if (row.status !== 'pending') return { ok: false, found: true, expired: false, answered: true, status: row.status };
  ...
  return { ok: true, found: true, expired: false, answered: false, status };
}
