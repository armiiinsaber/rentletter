...
export async function generateApplicantInsight({ application, listing, verificationRun }) {
  ...
  try {
    const message = await anthropic.messages.create({
      model: 'claude-sonnet-4-6',
      max_tokens: 500,
      // Cached static system block (same caveat as the doc-analysis prompt: below Sonnet 4.6's
      // 2048-token cache minimum today, so this is a free no-op until the prompt grows).
