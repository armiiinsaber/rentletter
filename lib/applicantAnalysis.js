...
// SERVER-ONLY shared helpers for the document intelligence routes.
...
// (/api/applicants/analyze-file, one file per request) and the tenant self-upload (/api/upload/analyze-file,
// one document per request) — so the persisted result is the same shape no matter who uploaded.
// PROCESS-AND-DISCARD: raw document bytes
...
import { nameOnDocMatches, resolveNameMatch, authorityComparisons, crossReference } from './documentAuthority.js';
...
// doc_verifications column being absent until the SQL is run.
...
// The per document matcher lives in lib/documentAuthority.js (nameOnDocMatches); the analysis
// uses resolveNameMatch there, which weighs each document by its authority on the name.
...
// Defence-in-depth: detect obvious OHRC protected-ground language so a caller can
...
(b) EXTRACT only SCREENABLE, FACTUAL fields that are PRINTED on each document. You extract; the software computes and compares. For a PAY DOCUMENT (pay stub, pay statement, earnings statement) copy exactly what is printed: the employer, the employee name as printed, the pay period start date, the pay period end date, the pay date, the GROSS pay for that period, the regular hourly rate if printed, the hours for the period if printed, and the pay frequency ONLY if the document prints it. Do NOT annualize. Do NOT compute a monthly or yearly figure. Do NOT multiply, average, or choose between stubs. Do NOT guess a frequency from the dates; leave payFrequency null unless the word is printed. Any field you cannot read off the page is null, never estimated. For an EMPLOYMENT LETTER or OFFER LETTER copy the employer, the employee name, the job title, the employment type (full-time/part-time/contract), the employment start date as printed (startDate), and the annual salary only if a yearly figure is literally printed (annualSalaryPrinted); otherwise null. For a T4 or NOTICE OF ASSESSMENT copy the employer and the total employment income printed for the year (annualSalaryPrinted). For a BANK STATEMENT copy the account holder name only. For a GOVERNMENT ID copy the name only.
- For a CREDIT REPORT read exactly this and nothing more: the credit SCORE (the number), the score band if shown, the bureau (Equifax / TransUnion / Experian), the report date, the name as printed, and the employer line if one is printed (copy it into employer; the software shows it as historical and never compares it). Read NOTHING else from a credit report: no accounts, no tradelines, no balances, no limits, no inquiries, no addresses, no payment history, no collections. The software does not use a credit report for anything it scores.
- Capture nothing beyond these screenable facts.
(c) Do NOT cross reference the documents against each other and do NOT compare them to the application. The software decides which document is the authority on each fact, annualizes every pay document, and compares employer, title, start date and income itself. Return no comparisons.
(d) Give a brief, neutral overall summary of what the documents are and a confidence level for how legible they were. The summary never weighs income against the application and never describes credit report contents beyond the score.
...
{
  "documents": [
    { "filename": "<the provided filename>", "documentType": "<pay stub | employment letter | credit report | bank statement | government ID | reference letter | tax document (T4) | notice of assessment | a clear specific custom label | Other / Unrecognized>", "unrecognized": <true if it is not a recognizable supporting document, else false>, "extracted": { "applicantName": <string|null, the name exactly as printed>, "employer": <string|null>, "employmentType": <string|null>, "jobTitle": <string|null>, "startDate": <string|null, employment or offer letter only, the start date as printed>, "periodStart": <string|null, the pay period start as printed>, "periodEnd": <string|null>, "payDate": <string|null>, "grossForPeriod": <number|null, the gross pay for that period only>, "regularRate": <number|null, hourly rate if printed>, "hours": <number|null, hours for the period if printed>, "payFrequency": <string|null, only if the word is printed>, "annualSalaryPrinted": <number|null, a letter's printed yearly salary, or a T4 / notice of assessment total employment income>, "documentDate": <string|null>, "creditScore": <number|null, credit report only>, "scoreBand": <string|null>, "bureau": <"Equifax" | "TransUnion" | "Experian" | string | null>, "reportDate": <string|null> }, "notes": "<short factual note; for Other / Unrecognized say what it appears to be; for a credit report, nothing beyond the score>" }
  ],
...
export async function runDocumentAnalysis({ files, application, listing }) {
  ...
  // The application's stated values never reach the model: every comparison is made in code
  // (lib/documentAuthority.js), so the model has nothing to agree with.
  content.push({ type: 'text', text: `Read ALL ${list.length} document(s) and return the STRICT JSON report now.` });
    ...
    const message = await anthropic.messages.create(
      {
        ...
        max_tokens: 3000,
        // Prompt caching on the static system block (volatile data stays in the user turn).
        // Note: Sonnet 4.6 only caches prefixes ≥2048 tokens; this prompt (~1.5k) is below
        // that today, so the marker is a free no-op that becomes live if the prompt grows.
        system: [{ type: 'text', text: DOC_ANALYSIS_SYSTEM_PROMPT, cache_control: { type: 'ephemeral' } }],
  ...
  // NAME-MATCH SAFEGUARD: do the names printed on the documents belong to THIS applicant? Each
  // document weighs by its authority on the name (lib/documentAuthority.js): matching ID is not
  // undone by a bank statement in another name; a clearly different person is still flagged.
  ...
  // EVERY COMPARISON IS CODE: the strongest document speaks for each fact, income is annualized
  // across every stub and weighed against the letter, and a credit report's employer line is
  // shown as historical, never compared. Anything the model returned under comparisons or
  // crossReference is ignored.
  const comparisons = authorityComparisons(documents, facts);
