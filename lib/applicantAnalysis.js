...
(b) EXTRACT only SCREENABLE, FACTUAL fields that are PRINTED on each document. You extract; the software computes. For a PAY DOCUMENT (pay stub, pay statement, earnings statement) copy exactly what is printed: the employer, the employee name as printed, the pay period start date, the pay period end date, the pay date, the GROSS pay for that period, the regular hourly rate if printed, the hours for the period if printed, and the pay frequency ONLY if the document prints it. Do NOT annualize. Do NOT compute a monthly or yearly figure. Do NOT multiply, average, or choose between stubs. Do NOT guess a frequency from the dates; leave payFrequency null unless the word is printed. Any field you cannot read off the page is null, never estimated. For an EMPLOYMENT LETTER or OFFER LETTER copy the annual salary only if a yearly figure is literally printed (annualSalaryPrinted); otherwise null. Also extract the employer name, employment type (full-time/part-time/contract) and job title when printed.
- For a CREDIT REPORT specifically, the credit SCORE is the single most important field, ALWAYS capture it if legible. Extract: creditScore (the number), scoreBand (the band label if shown), bureau (Equifax / TransUnion / Experian), reportDate, and only clearly stated account facts.
- Capture nothing beyond these screenable facts.
(c) CROSS-REFERENCE across the documents: does the applicant name match across documents and the ID? does the employer match between a pay stub and an employment letter? Report each as consistent or a discrepancy with a short factual detail. Do not cross reference income figures; the software does that arithmetic.
(d) COMPARE the documents to the application's STATED values (provided below) for EMPLOYER and JOB TITLE only. For each, say whether the documents match, are close, mismatch, or were not found. Do NOT compare income: the software annualizes every pay document itself from the fields you extracted.
...
{
  "documents": [
    { "filename": "<the provided filename>", "documentType": "<pay stub | employment letter | credit report | bank statement | government ID | reference letter | a clear specific custom label | Other / Unrecognized>", "unrecognized": <true if it is not a recognizable supporting document, else false>, "extracted": { "applicantName": <string|null, the employee name exactly as printed>, "employer": <string|null>, "employmentType": <string|null>, "jobTitle": <string|null>, "periodStart": <string|null, the pay period start as printed>, "periodEnd": <string|null>, "payDate": <string|null>, "grossForPeriod": <number|null, the gross pay for that period only>, "regularRate": <number|null, hourly rate if printed>, "hours": <number|null, hours for the period if printed>, "payFrequency": <string|null, only if the word is printed>, "annualSalaryPrinted": <number|null, employment or offer letter only, only if a yearly figure is printed>, "documentDate": <string|null>, "creditScore": <number|null, credit report only>, "scoreBand": <string|null>, "bureau": <"Equifax" | "TransUnion" | "Experian" | string | null>, "reportDate": <string|null>, "accountsCount": <number|null>, "delinquencies": <string|null, factual presence only>, "collections": <string|null, factual presence only> }, "notes": "<short factual note; for Other / Unrecognized say what it appears to be>" }
  ],
  "crossReference": [ { "field": "<e.g. Applicant name | Employer | Income>", "status": "consistent" | "discrepancy", "detail": "<short factual detail>" } ],
  "comparisons": [ { "field": "Employer" | "Job title", "stated": <string|null>, "found": <string|null>, "status": "match" | "close" | "mismatch" | "not_found" } ],
...
export async function runDocumentAnalysis({ files, application, listing }) {
  ...
  // INCOME IS ARITHMETIC, DONE HERE (lib/incomeAnnualize.js): every pay document of this call,
  // partial periods detected from the hours, the median full period annualized. Any income row
  // the model returned anyway is dropped.
  const modelComparisons = (Array.isArray(parsed.comparisons) ? parsed.comparisons : []).filter((c) => c && !/income/i.test(String(c.field || '')));
  const comparisons = [...modelComparisons, incomeComparison(documents, facts.statedAnnualIncome)];
