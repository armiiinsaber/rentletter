...
// (pages/dashboard.js), so the first paint of the dashboard already has the feed and nothing
...
export async function notificationsFor({ supabase, admin, userId, listings = null }) {
  ...
  const listingName = Object.fromEntries(owned.map((l) => [String(l.id), displayLabel(l, 'your listing')]));
  ...
  const [profileRes, linksFirst, docsRes] = await Promise.all([
    ...
    linksQ('id, listing_id, application_id, created_at, decision_changed_at, withdrawn_at, state'),
  ...
  // The same before db/002 has run (no state column): first without the state, then without both.
  let linksRes = linksFirst;
  if (linksRes.error) linksRes = await linksQ('id, listing_id, application_id, created_at, withdrawn_at');
  ...
  for (const r of links) {
    ...
    const standing = applicantStanding(r);
    if (standing.withdrawn && standing.withdrawnSince) {
      const wTs = new Date(standing.withdrawnSince).getTime();
