...
import { applicantState, STATE_LABELS } from './applicantState.js';
import { isWithdrawn, isSetAside } from './listingApplicantsVocabulary.js';
import { listingOpen } from './listingState.js';
import { listingStanding } from './application-state.js';

// The count words come from the one label map (lib/applicantState.js STATE_LABELS.*.count).
const ORDER = [['verified', STATE_LABELS.verified.count], ['matched', STATE_LABELS.matched.count], ['check', STATE_LABELS.checked.count], ['waiting', STATE_LABELS.requested.count], ['none', STATE_LABELS.new.count], ['sent', STATE_LABELS.sent.count]];
...
export function listingStateLine(listing, applicants, { maxStates = 2 } = {}) {
  if (listing && !listingOpen(listing)) {
    ...
    const word = standing.withdrawn ? 'Closed' : 'Rented';
    const when = standing.closedAt ? ` · ${shortDate(standing.closedAt)}` : '';
