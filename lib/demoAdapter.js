// lib/demoAdapter.js — CLIENT-ONLY. The /demo/sandbox adapter: the SAME dashboard components
...
export const DEMO_PATHS = { home: '/demo/sandbox', listing: (id) => `/demo/sandbox?listing=${encodeURIComponent(id)}`, profile: '/demo/sandbox#branding', signin: '/' };
...
export function createDemoAdapter(initial) {
  ...
  const routes = {
    ...
    'POST /api/referrals/assign': (q, b) => {
      ...
      (state.applicantsByListing[listing.id] = state.applicantsByListing[listing.id] || []).push({ linkId, decisionStatus: 'ranked', decisionPriority: null, decisionNotes: '', decisionReasonCode: null, decisionChangedAt: null, addedVia: 'referral', reviewedAt: null, reviewTracking: true, application: row, docVerifications: [], docArchived: [], aiInsight: null });
    ...
    'POST /api/listings/send-report': (q, b) => { const l = state.listings.find((x) => x.id === b.listingId); if (!l?.landlord_email) return json(400, { error: 'This listing has no landlord email.' }); return json(200, { ok: true, sentTo: l.landlord_email, preview: true }); },
  ...
  const decisionsFor = (listingId) => Object.fromEntries((state.applicantsByListing[listingId] || []).map((a) => [a.linkId, a.decisionStatus]));
