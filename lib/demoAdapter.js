...
export function createDemoAdapter(initial) {
  ...
  const live = (listingId, apps) => { const l = state.listings.find((x) => x.id === listingId) || {}; return (apps || []).map((a) => { const s = withLiveScore(a, l); const r = (state.docRequests || {})[a.linkId] || null; return { ...s, docRequest: r ? { status: r.status, requestedAt: r.requestedAt || null, receivedAt: r.receivedAt || null, nudgedAt: Array.isArray(r.nudgedAt) ? r.nudgedAt : [] } : null, application: { ...s.application, fit: fitFor(s, l) } }; }); };
  ...
  const routes = {
    ...
    'POST /api/listings/send-report': (q, b) => { const l = state.listings.find((x) => x.id === b.listingId); if (!l?.landlord_email) return json(400, { error: 'This listing has no landlord email.' }); const sentAt = new Date().toISOString(); (state.applicantsByListing[l.id] || []).forEach((a) => { if (!a.withdrawnAt) a.lastSentAt = sentAt; }); record('report_sent', { listingId: l.id, payload: { landlordEmail: l.landlord_email, landlordName: l.landlord_name || null } }); return json(200, { ok: true, sentTo: l.landlord_email, preview: true }); },
  ...
  const adapter = {
    ...
    getState: () => ({ ...state, applicantsByListing: liveByListing() }), subscribe: (fn) => { listeners.add(fn); return () => listeners.delete(fn); },
