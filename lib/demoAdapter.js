...
export function createDemoAdapter(initial) {
  ...
  const routes = {
    'GET /api/assistant/signals': () => json(200, { signals: { listings: state.listings, applicantsByListing: state.applicantsByListing, notifications: state.notifications, referralsInbox: state.referralsInbox, referralsSent: Object.values(state.referralsSent), latestEventAt: (state.events || [])[0]?.created_at || null, loaded: true } }),
