...
export function freshState() {
  return {
    ...
    docRequests: { 'demo-link-2': { status: 'requested', url: 'https://rentletter.ca/upload/demo0000000000000000000000000002', requestedAt: new Date(Date.now() - 4 * 86400000).toISOString(), receivedAt: null } },
...
export function createDemoAdapter(initial) {
  ...
  const live = (listingId, apps) => { const l = state.listings.find((x) => x.id === listingId) || {}; return (apps || []).map((a) => { const s = withLiveScore(a, l); const r = (state.docRequests || {})[a.linkId] || null; return { ...s, docRequest: r ? { status: r.status, requestedAt: r.requestedAt || null, receivedAt: r.receivedAt || null } : null, application: { ...s.application, fit: fitFor(s, l) } }; }); };
