...
//     score, label, A, E, R, ratio, incomeUsed, incomeSource, criteria, confirmations, model: 'fit-v2' }
//
...
export function readVerification(verification, { ignoreName = false } = {}) {
  ...
  if (!ignoreName && (r.nameMatch === 'mismatch' || r.nameMatch === 'unclear')) return { state: r.nameMatch, incomeMatched: false, employerMatched: false, incomeFound: null };
...
export function computeFit({ application, listing, verification, confirmations } = {}) {
  ...
  // A landlord reference on file is worth 1.0; the realtor having called that landlord is worth 1.5.
  let R = clamp(2.5 + (landlordConfirmed ? 1.5 : landlordRef ? 1.0 : 0) + 0.5 * Math.min(refs, 2) + (tenureMet ? 0.5 : 0), 1, 5);
  ...
  const label = employerConfirmed ? 'verified' : E >= 4.5 ? 'docs match' : 'stated';
  ...
  return { score, label, A, E, R, ratio, incomeUsed: householdIncome, incomeSource, criteria, confirmations: conf, model: FIT_MODEL };
}
