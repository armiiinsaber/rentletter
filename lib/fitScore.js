...
//                 { id?, employer?, landlord?, landlord_reference?, reference? : { at, by } }. The realtor verifies;
...
import { DEFAULT_RENT_SHARE_CAP, derivedMinIncome, sameAsCap } from './listingForm.js';

export const FIT_MODEL = 'fit-v2';

// The old lib/scoring.js is gone; its two helpers live here. round1
...
export function readVerification(verification, { ignoreName = false } = {}) {
  ...
  if (!r || typeof r !== 'object' || docs.length === 0) return { state: 'none', incomeMatched: false, employerMatched: false, incomeFound: null, incomeMismatch: false, employerMismatch: false, incomeClose: false, employerAlsoSeen: [], employerSince: null };
  ...
  if (!ignoreName && (r.nameMatch === 'mismatch' || r.nameMatch === 'unclear')) return { state: r.nameMatch, incomeMatched: false, employerMatched: false, incomeFound: null, incomeMismatch: false, employerMismatch: false, incomeClose: false, employerAlsoSeen: [], employerSince: null };
  ...
  // Only a mismatch contradicts the application. A close status (within 15%, lib/documentAuthority.js)
  // is not a contradiction: the label stays docs match and the checklist shows both figures.
  ...
  // The employer status is recomputed from the stored stated and found values through the current
  // normalization (lib/employerName.js), so an older report's "Livenation" against "Live Nation Canada Inc"
  // reads matched today without a re analysis.
  ...
  // incomeFound is the annual the code computed (lib/incomeAnnualize.js); older reports carry a bare figure in found.
  const incomeRow = comparisons.find((c) => c && /income/i.test(String(c.field || '')));
  // The Employer row is resolved by document authority (lib/documentAuthority.js): alsoSeen carries
  // lower sources that differ (a credit report's employer line, historical), since carries the
  // letter's start date. Both are shown on the checklist and neither is a Fit input.
  const employerRow = comparisons.find((c) => c && /employer/i.test(String(c.field || '')));
  return {
...
// Joint income: a co applicant's income was added to the applicant's. Every reader that shows a
// household figure labels it "(joint)" through incomeIsJoint; householdIncomeOf is that figure.
...
// The minimum income Fit uses: the stored one, unless it equals rent × 12 / cap within 2%, in
// which case it is the cap said twice and reads as no minimum (older listings derived it that way).
export function effectiveMinIncome(l) {
  const min = num(l && l.pref_min_annual_income);
  if (!(min > 0)) return 0;
  const derived = derivedMinIncome(num(l && l.monthly_rent), capOf(l));
  return sameAsCap(min, derived) ? 0 : min;
}
...
export function computeFit({ application, listing, verification, confirmations } = {}) {
  ...
  const idConfirmed = confirmationCounts(conf.id, editedAt), employerConfirmed = confirmationCounts(conf.employer, editedAt), landlordConfirmed = !!conf.landlord || !!conf.landlord_reference; // the call or the emailed answers (lib/referenceStore.js); the answers themselves never move the number
  ...
  const minIncome = effectiveMinIncome(l); // 0 when the stored minimum only restates the cap
  ...
  // Tenure is the stated years at job for now. A later change that trusts the letter would read the
  // Employer row's startDate (lib/documentAuthority.js employerComparison) here instead.
  ...
  // A close income keeps 'docs match'; the number itself still rests on the stated income.
  ...
  // the rest when set. value and rule carry the numbers the landlord report prints.
  const criteria = [];
  ...
  if (minIncome > 0) criteria.push({ key: 'pref_min_annual_income', label: 'Minimum income', status: householdIncome >= minIncome ? 'met' : 'missed', detail: householdIncome >= minIncome ? `Income ${moneyK(householdIncome)} · your min ${moneyK(minIncome)}` : `Below your ${moneyK(minIncome)} minimum`, value: householdIncome, rule: minIncome });
  if (minYears > 0) criteria.push({ key: 'pref_min_years_at_job', label: 'Tenure', status: tenureMet ? 'met' : 'missed', detail: `${years} yrs at employer · your min ${minYears}`, value: years, rule: minYears });
  ...
  return { score, scoreExact, label, A, E, R, ratio, incomeUsed: householdIncome, incomeJoint: co > 0, incomeSource, criteria, confirmations: conf, parts: { tenure, tenancy, refs, landlordRef, yearsAtJob: years, yearsAtPrevious: yearsPrev, refCount }, evidence, model: FIT_MODEL };
}
...
export function fitReason(lower, upper) {
  ...
  if (dE > 0) {
    if (!le.hasReport && !le.employerConfirmed) facts.push([STATE_LABELS.new.reason, dE]);
    else if (le.edited) facts.push([STATE_LABELS.edited.reason, dE]);
    else if (le.contradicted) facts.push([STATE_LABELS.checked.reason, dE]);
    else if (ue.employerConfirmed && !le.employerConfirmed) facts.push([STATE_LABELS.matched.reason, dE]);
  }
