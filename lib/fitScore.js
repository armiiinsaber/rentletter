...
export function computeFit({ application, listing, verification, confirmations } = {}) {
  ...
  const idConfirmed = !!conf.id, employerConfirmed = !!conf.employer, landlordConfirmed = !!conf.landlord;
  const rent = num(l.monthly_rent);
  const v = readVerification(verification, { ignoreName: idConfirmed });
  ...
  const label = employerConfirmed ? 'verified' : !hasReport ? 'stated' : contradicted ? 'check docs' : v.incomeMatched ? 'docs match' : 'stated';
  ...
  return { score, scoreExact, label, A, E, R, ratio, incomeUsed: householdIncome, incomeSource, criteria, confirmations: conf, parts: { tenure, tenancy, refs, landlordRef, yearsAtJob: years, yearsAtPrevious: yearsPrev, refCount }, model: FIT_MODEL };
}
...
// One line, at most eight words, naming what places `lower` below `upper` in the ranking, from
// screenable record facts only: never the income level. null when the two are within 0.02.
export function fitReason(lower, upper) {
  ...
  const facts = [];
  if (lower.label === 'check docs' && upper.label !== 'check docs') facts.push(['documents not matched', 0.3 * Math.max(upper.E - lower.E, 0.1)]);
  if (lower.ratio != null && upper.ratio != null && lower.ratio > upper.ratio) facts.push(['higher rent share', Math.max(0.5 * (upper.A - lower.A), 0.01)]);
  ...
  const missed = (f) => new Set((f.criteria || []).filter((c) => c.status === 'missed').map((c) => c.key));
  const lm = missed(lower), um = missed(upper);
  if ([...lm].some((k) => !um.has(k))) facts.push(['below a criterion', 0.05]);
