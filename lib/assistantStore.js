...
// One listing's applicants were refetched by the page (after an analysis): the badge follows.
export const patchSignalsListing = (listingId, applicants) => { if (!state.signals || !listingId || !Array.isArray(applicants)) return; patch({ signals: { ...state.signals, applicantsByListing: { ...(state.signals.applicantsByListing || {}), [listingId]: applicants } } }); };
// A listing row changed (status, closed_at, rented_link_id): the Next list and bell recompute from it.
export const patchSignalsListingRow = (listingId, fields) => { if (!state.signals || !listingId || !fields) return; patch({ signals: { ...state.signals, listings: (state.signals.listings || []).map((l) => (l.id === listingId ? { ...l, ...fields } : l)) } }); };
