...
import { isApplicationState, oldColumnsOf } from './application-state';
...
export function demoReportContext(listingId, decisionsJson) {
  ...
  const applicants = markDuplicates(buildDemoApplicants()[listing.id] || []).map((a) => {
    const d = decisions[a.linkId];
    const status = d && typeof d === 'object' && DECISION_STATUS_VALUES.includes(d.status) ? d.status : oldColumnsOf(a).decision;
    const withdrawnAt = d && typeof d === 'object' && typeof d.withdrawnAt === 'string' && d.withdrawnAt ? d.withdrawnAt : null;
    // The link carries the sandbox's state beside the old columns; an older link carries none and
    // the row falls back to them.
    const row = { ...a, decisionStatus: status, withdrawnAt, state: d && typeof d === 'object' ? (isApplicationState(d.state) ? d.state : null) : a.state };
