...
// linkId → decision status (validated against the fixture, so the PDF reflects the sandbox's
// current set-asides). Nothing is stored.
...
const STATUSES = new Set(['ranked', 'set_aside', 'withdrawn']);
...
export function demoReportContext(listingId, decisionsJson) {
  ...
  const applicants = (buildDemoApplicants()[listing.id] || []).map((a) => {
    ...
    return typeof d === 'string' && STATUSES.has(d) ? { ...a, decisionStatus: d } : a;
  });
...
export function demoReportText(ctx) {
  ...
  ctx.active.forEach((row, i) => {
    ...
    out.push(`${i + 1}. ${a.full_name}${row.decisionPriority === 'finalist' ? '  ★ finalist' : ''}`);
