...
// Read only: does the link exist? Used by the confirm page on GET, which writes nothing.
export async function peekMagicLink(t) {
  if (!t || String(t).length < 20) return null;
  const rec = await kvGet(`tmagic:${hash(t)}`);
  return rec && rec.email ? { email: rec.email } : null;
}
...
export async function peekEmailChange(t) {
  if (!t || String(t).length < 20) return null;
  const rec = await kvGet(`temail:${hash(t)}`);
  return rec && rec.profileId ? { newEmail: rec.newEmail } : null;
}
