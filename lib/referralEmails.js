...
${P(`<strong style="color:#0f0f10;">Nothing is shared until you say so.</strong> Your application details (employment, income, rental history, household, references, and your intro) would go to ${to}. Documents are never shared.`)}
...
${P('Open your dashboard, assign them to one of your listings, and they&rsquo;re ranked against that unit like any other applicant, no retyping, no re collecting.')}
...
<tr><td style="padding-bottom:28px;">
${P(`<strong style="color:#0f0f10;">${esc(applicantName || 'An applicant')}</strong> approved sharing their full rental application with you${fromBrokerage ? `, referred by ${esc(fromName)} at ${esc(fromBrokerage)}` : ''}.${note ? ` Their note: &ldquo;${esc(note)}&rdquo;` : ''}`)}
...
<tr><td style="padding-bottom:28px;">
${P(approved ? `Their application is now with <strong style="color:#0f0f10;">${esc(toName || 'the receiving realtor')}</strong>. The referral is recorded on both sides.` : 'That&rsquo;s the whole message, no reason is collected, and there&rsquo;s nothing to follow up. Your own listing and application are unaffected.')}
