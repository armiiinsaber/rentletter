...
export function verificationFacts(docVerifications) {
  ...
  if (!r || docs.length === 0) return { documents: false, incomeVerified: false, employmentVerified: false };
  if (r.nameMatch === 'mismatch' || r.nameMatch === 'unclear') return { documents: true, incomeVerified: false, employmentVerified: false };
  ...
  return { documents: true, incomeVerified: match(/income/i), employmentVerified: match(/employer/i) };
}
...
// Income clause: what is known about the money, and whether documents back it.
function incomeClause(f) {
  if (f.incomeVerified) return f.multiple ? `Verified income at ${fmtMultiple(f.multiple)} rent` : 'Verified income';
