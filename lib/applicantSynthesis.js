...
// Deterministic, same conventions as lib/actions.js: pure rules over data the dashboard has
...
export function verificationFacts(docVerifications) {
  ...
  // Only a mismatch contradicts the application; a close status (within 15%) reads as unconfirmed,
  // the same way lib/fitScore.js readVerification reads it.
  const differs = (re) => !match(re) && comparisons.some((c) => re.test(String(c.field || '')) && c.status === 'mismatch');
  // The employer status is recomputed from the stored row (lib/employerName.js), as lib/fitScore.js reads it.
  const employerNow = employerRowStatus(comparisons.find((c) => c && /employer/i.test(String(c.field || ''))));
  return { documents: true, incomeVerified: match(/income/i), employmentVerified: employerNow === 'match', nameMismatch: false, incomeMismatch: differs(/income/i), employerMismatch: employerNow === 'mismatch' };
}
