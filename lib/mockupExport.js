...
// SVG <foreignObject> inconsistently — WebKit draws them as hard offset slabs. So the DOM is
// captured with shadows OFF on a transparent background (class `mk-exporting`, CSS in
// pages/admin/mockups.js), and the export is assembled in Canvas 2D:
//   1. the canvas background (gradient read from data-export-bg, drawn with createRadialGradient)
//   2. a soft shadow under every device (ctx.shadowBlur on each shell's rounded rect)
//   3. the captured devices + content on top.
const SHADOW = {
...
function paintShadow(ctx, stage, pixelRatio) {
  // EVERY device on the stage casts a canvas shadow (the film has a laptop AND a phone). CSS
  // box-shadows are stripped during capture: WebKit rasterizes blurred shadows inside a
  // foreignObject image as hard offset slabs. Each shadow is built on a scratch canvas: fill the
  // casting shape with shadow enabled, then PUNCH THE SHAPE'S INTERIOR OUT (destination-out) so
  // only the outer blur ring lands on the export — the fill must never show, because a device
  // does not fully cover its casting shape (a laptop lid is narrower than its base, and the
  // film's phone is semi-transparent while entering/leaving). The ring is also weighted by the
  // device's effective opacity so a fading phone fades its shadow with it.
  const sr = stage.getBoundingClientRect();
  const scratch = document.createElement('canvas'); scratch.width = ctx.canvas.width; scratch.height = ctx.canvas.height;
  const sc = scratch.getContext('2d');
  for (const shell of stage.querySelectorAll('.df-shell')) {
    const device = shell.closest('.df'); const base = device?.querySelector('.df-base');
    const r = shell.getBoundingClientRect(); const br = base?.getBoundingClientRect();
    ...
    let alpha = 1; for (let el = device; el && el !== stage; el = el.parentElement) { const o = parseFloat(getComputedStyle(el).opacity); if (!Number.isNaN(o)) alpha *= o; }
    if (alpha <= 0.01) continue;
    sc.clearRect(0, 0, scratch.width, scratch.height);
    for (const s of SHADOW[stageTone(stage)]) {
      sc.save();
      sc.shadowColor = s.color; sc.shadowBlur = s.blur * w; sc.shadowOffsetX = 0; sc.shadowOffsetY = s.dy * w;
      sc.fillStyle = '#101012';
      roundRect(sc, x, y, w, h, radius); sc.fill();
      sc.restore();
    }
    sc.save(); sc.globalCompositeOperation = 'destination-out'; sc.fillStyle = '#000';
    roundRect(sc, x, y, w, h, radius); sc.fill(); sc.restore();
    ctx.save(); ctx.globalAlpha = alpha; ctx.drawImage(scratch, 0, 0); ctx.restore();
  }
}
