// lib/pipeline.js  SERVER ONLY (takes the service role client). Pipeline: the reads and the
...
// The list: three reads. The consent rows for this profile (pending and consented), their applications, the junction
...
export async function readPeopleSource({ profileId, admin, now = new Date() } = {}) {
  ...
  const { data: consents, error } = await admin.from('pipeline_consents').select('*').eq('profile_id', profileId).in('status', ['pending', 'consented']);
...
// The token is consumed on the first read: the record moves to prefill:{token}:claimed with a
// nonce, the page sets that nonce as a cookie scoped to the apply path, and only a later read that
// carries the same nonce sees the row again. Anyone else who opens the link gets the empty form.
export const PREFILL_COOKIE = 'rl_prefill';
export const claimedKey = (token) => `${prefillKey(token)}:claimed`;
export async function readPrefill(admin, token, { nonce = null } = {}) {
  if (!admin || !isPrefillToken(token)) return null;
  let rec = await kvGetJson(prefillKey(token));
  let setNonce = null;
  if (rec && rec.applicationId) {
    setNonce = crypto.randomBytes(16).toString('base64url');
    const ok = await kvSetJson(claimedKey(token), { ...rec, nonce: setNonce }, PREFILL_TTL);
    if (!ok) return null; // KV down: nothing renders prefilled rather than rendering without the claim
    await kvDel(prefillKey(token));
  } else {
    const claimed = await kvGetJson(claimedKey(token));
    if (!claimed || !nonce || String(claimed.nonce) !== String(nonce)) return null;
    rec = claimed;
  }
  ...
  return { application: row, consentId: rec.consentId || null, nonce: setNonce };
}
export const consumePrefill = async (token) => { if (!isPrefillToken(token)) return false; await kvDel(prefillKey(token)); await kvDel(claimedKey(token)); return true; };
