...
import { reportEvent } from './clientEvents';
const sb = async (ctx) => { if (ctx?.adapter?.supabase) return ctx.adapter.supabase(); const { getSupabaseBrowserClient } = await import('./supabase/client'); return getSupabaseBrowserClient(); };
...
export const ACTIONS = {
  ...
  mark_finalist: {
    ...
    execute: async (ctx, p) => { const client = await sb(ctx); const { error } = await client.from('listing_applicants').update({ decision_priority: p.finalist === false ? DECISION_PRIORITY.NORMAL : DECISION_PRIORITY.TOP }).eq('id', p.linkId); if (error) throw new Error(error.message); reportEvent(ctx?.adapter, { type: 'applicant_marked_finalist', linkId: p.linkId, payload: { removed: p.finalist === false } }); return { text: p.finalist === false ? 'Finalist mark removed.' : 'Marked as a finalist.', patch: { linkId: p.linkId, decisionPriority: p.finalist === false ? DECISION_PRIORITY.NORMAL : DECISION_PRIORITY.TOP } }; },
  },
  refer_applicant: {
  ...
  update_preferences: {
    ...
    execute: async (ctx, p) => { const client = await sb(ctx); const { error } = await client.from('listings').update(p.patch).eq('id', p.listingId); if (error) throw new Error(error.message); reportEvent(ctx?.adapter, { type: 'listing_updated', listingId: p.listingId, payload: { what: 'preferences' } }); return { text: 'Preferences saved. Fit indicators update on the listing page.', patch: { listingId: p.listingId, ...p.patch } }; },
  },
};
