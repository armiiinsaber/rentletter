...
export const ACTIONS = {
  ...
  email_report: {
    ...
    execute: async (ctx, p) => { const j = await post(ctx, '/api/listings/send-report', { listingId: p.listingId }); return { text: `Sent to ${j.sentTo}.` }; },
  },
  mark_finalist: {
  ...
  update_preferences: {
    ...
    execute: async (ctx, p) => { const client = await sb(ctx); const { error } = await client.from('listings').update(p.patch).eq('id', p.listingId); if (error) throw new Error(error.message); return { text: 'Preferences saved. Fit indicators update on the listing page.', patch: { listingId: p.listingId, ...p.patch } }; },
  },
};
