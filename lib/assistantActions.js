...
export const ACTIONS = {
  ...
  mark_finalist: {
    ...
    execute: async (ctx, p) => { const client = await sb(ctx); const { error } = await client.from('listing_applicants').update({ decision_priority: p.finalist === false ? DECISION_PRIORITY.NORMAL : DECISION_PRIORITY.TOP }).eq('id', p.linkId); if (error) throw new Error(error.message); return { text: p.finalist === false ? 'Finalist mark removed.' : 'Marked as a finalist.', patch: { linkId: p.linkId, decisionPriority: p.finalist === false ? DECISION_PRIORITY.NORMAL : DECISION_PRIORITY.TOP } }; },
  },
  refer_applicant: {
