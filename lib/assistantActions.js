...
export const PREF_FIELDS = {
  ...
  pref_min_lease_term_months: { label: 'Minimum lease term', type: 'number', fmt: (v) => `${v} months` },
  pref_max_occupants: { label: 'Maximum occupants', type: 'number', fmt: (v) => `${v}` },
  ...
  pref_guarantor_accepted: { label: 'Guarantor accepted', type: 'boolean', fmt: (v) => (v ? 'yes' : 'no') },
};

const post = async (url, body) => { const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const j = await r.json().catch(() => ({})); if (!r.ok) throw new Error(j?.error || 'Request failed.'); return j; };
...
export const ACTIONS = {
  create_invite_link: {
    ...
    execute: async (ctx, p) => { const j = await post('/api/listings/invite', { listingId: p.listingId }); return { text: `Invite link ready: ${j.url || j.inviteUrl || '(see listing page)'}`, url: j.url || j.inviteUrl }; },
  },
  request_documents: {
  ...
  email_report: {
    ...
    execute: async (ctx, p) => { const j = await post('/api/listings/send-report', { listingId: p.listingId }); return { text: `Sent to ${j.sentTo}.` }; },
  },
  mark_finalist: {
    ...
    execute: async (ctx, p) => { const { getSupabaseBrowserClient } = await import('./supabase/client'); const sb = getSupabaseBrowserClient(); const { error } = await sb.from('listing_applicants').update({ decision_priority: p.finalist === false ? null : 'finalist' }).eq('id', p.linkId); if (error) throw new Error(error.message); return { text: p.finalist === false ? 'Finalist mark removed.' : 'Marked as a finalist.', patch: { linkId: p.linkId, decisionPriority: p.finalist === false ? null : 'finalist' } }; },
  },
  refer_applicant: {
    ...
    execute: async (ctx, p) => { const j = await post('/api/referrals/create', { listingId: p.listingId, linkId: p.linkId, toName: p.toName, toEmail: p.toEmail, note: p.note || '' }); return { text: `Consent request sent. You’ll see “Pending applicant approval” on the card until they answer.`, referral: j.referral }; },
  },
  update_preferences: {
    ...
    execute: async (ctx, p) => { const { getSupabaseBrowserClient } = await import('./supabase/client'); const sb = getSupabaseBrowserClient(); const { error } = await sb.from('listings').update(p.patch).eq('id', p.listingId); if (error) throw new Error(error.message); return { text: 'Preferences saved. Fit indicators update on the listing page.', patch: { listingId: p.listingId, ...p.patch } }; },
  },
};
