...
import { FIT_LINE } from './reportSnapshot.js';
...
export function reportLines(payload) {
  ...
  const header = {
    ...
    address: displayAddress(l, 'Listing'),
  ...
  const blocks = (P.applicants || []).map((a) => {
    ...
    return {
      ...
      // The math under the numbers: [status, text] per rule in force (lib/reportSnapshot.js criteriaRows).
      criteria: Array.isArray(a.criteria) ? a.criteria.map((c) => [c.status, c.text]) : [],
      numbers: [['INCOME', n.annualIncome != null ? `${money(n.annualIncome)}${n.incomeJoint ? ' (joint)' : ''}` : 'not given'], ['RENT SHARE', n.rentSharePct != null ? `${Math.round(n.rentSharePct)}%` : 'unknown'], ['AT JOB', n.yearsAtJob ? `${n.yearsAtJob} yr${n.yearsAtJob === 1 ? '' : 's'}` : 'not given'], ['REFERENCES', String(n.references || 0)]],
    };
  });
  const footer = {
...
export async function buildLandlordReportPdf({ payload }) {
  ...
  const blockHeight = (b) => {
    ...
    if (b.criteria.length) h += 6 + Math.ceil(b.criteria.length / 2) * SMALL; // the math, two columns
    h += 14; // air below, then the hairline
    return h;
  };
  const footerLines = footer.criteria ? wrapWidth(footer.criteria, inter, 9.5, W) : [];
  const fitLines = wrapWidth(footer.fitLine, inter, 8.5, W);
  const footerHeight = 12 + footerLines.length * SMALL + 8 + 8 + (footer.signature ? SMALL : 0) + 8.5 * 1.3 + 2 + 4 + fitLines.length * 8.5 * 1.3;
  ...
  blocks.forEach((b, i) => {
    ...
    y -= 3 + 10.5;
    // the math: two columns, met with the red tick, missed with an ink dot, unverified with nothing
    if (b.criteria.length) {
      const rowsTall = Math.ceil(b.criteria.length / 2); const half = W / 2;
      const top = y - 6;
      b.criteria.forEach(([status, line], k) => {
        const col = Math.floor(k / rowsTall), rowIx = k % rowsTall;
        const x = M + col * half; y = top - 9.5 - rowIx * SMALL;
        if (status === 'met') tick(x, y + 1); else if (status === 'missed') page.drawCircle({ x: x + 3.5, y: y + 3.2, size: 1.7, color: INK });
        text(line, x + 14, 9.5, inter, INK);
      });
      y = top - 9.5 - (rowsTall - 1) * SMALL;
    }
  ...
  y -= 4 + 8.5 * 1.3;
  fitLines.forEach((ln) => { text(ln, M, 8.5, inter, INK_MUTE); y -= 8.5 * 1.3; });
...
export async function buildVerificationPdf({ profile, listing, applicantName, verification, fonts }) {
  ...
  const unitName = safe(displayLabel(listing));
