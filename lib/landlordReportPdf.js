...
export async function buildLandlordReportPdf({ profile, listing, active = [], setAside = [], fonts }) {
  ...
  const unitMeta = [
    ...
    listing?.bedrooms ? safe(formatUnit(listing.bedrooms)) : null,
  ].filter(Boolean).join('  ·  ');
  ...
  // Province-aware compliance footer (ON: Ontario Human Rights Code; BC: British Columbia).
  const hrCode = humanRightsCodeName(profile?.province);
  const drawFooter = (pg) => {
    pg.drawLine({ start: { x: MARGIN, y: 58 }, end: { x: PW - MARGIN, y: 58 }, thickness: 0.5, color: DIVIDER });
    pg.drawText(`Self reported data - verify references independently. Screening must comply with the ${hrCode}.`, {
      x: MARGIN, y: 44, size: 8, font: bodyFont, color: INK_MUTE,
    });
  ...
  // prevRow: the applicant ranked directly above (active list only); one line names what places
  // this one below it (lib/fitScore.js fitReason), omitted when the two are within 0.02.
  const drawApplicant = (row, rankNum, highlight, setAsideReasonText, prevRow = null) => {
    const app = row.application || {};
    const reasonLine = prevRow && !setAsideReasonText ? fitReason(app.fit, prevRow.application?.fit) : null;
    const notes = fitNotes(app.scorecard);
    // The realtor's set aside notes are private: only the reason's label (lib/setAsideReasons.js) prints.
    const confirmedLine = confirmedSummary(row.confirmations);
    // NOTE: the GROUP shortlist is RANKING ONLY — no per-applicant document-verification here.
    // Verification is a separate Stage-2, single-applicant confirmation (see buildVerificationPdf).
    const blockH = 70 + notes.length * 13 + (setAsideReasonText ? 14 : 0) + (confirmedLine ? 13 : 0) + (reasonLine ? 13 : 0);
    ensureSpace(blockH);
    ...
    y -= 18;
    const empType = { 'full-time': 'full-time', 'part-time': 'part-time', contract: 'contract', 'self-employed': 'self-employed' }[app.employment_type] || null;
    const role = [app.job_title, app.employer].filter(Boolean).map(safe).join(' at ') + (empType ? ` (${empType})` : '');
    page.drawText((role || 'Role not listed').slice(0, 80), { x: MARGIN + 28, y, size: 11, font: bodyFont, color: INK_SOFT });
    y -= 16;
    const facts = [];
    // Income is GROSS; the after-tax figure (tenant-stated or estimated) is labelled so the two
    // can't be misread for each other.
    if (app.annual_income) facts.push(`$${Number(app.annual_income).toLocaleString()}/yr before tax`);
    if (app.net_income) facts.push(`${app.net_income_source === 'stated' ? '' : 'est. '}$${Number(app.net_income).toLocaleString()}/yr after tax`);
    ...
    if (confirmedLine) {
      page.drawText(safe(confirmedLine).slice(0, 100), { x: MARGIN + 28, y, size: 9.5, font: bodyBold, color: INK_SOFT });
      y -= 13;
    }
    if (reasonLine) {
      page.drawText(safe(`Below the one above: ${reasonLine}`).slice(0, 100), { x: MARGIN + 28, y, size: 9.5, font: bodyFont, color: INK_SOFT });
      y -= 13;
    }
    y -= 10;
  ...
  if (total === 0) {
    ...
    if (topN.length) {
      ...
      topN.forEach((row, i) => drawApplicant(row, i + 1, true, null, i ? topN[i - 1] : null));
    }
    if (rest.length) {
      drawSectionHeader('ALSO RANKED', INK_MUTE, null);
      rest.forEach((row, i) => drawApplicant(row, i + 6, false, null, i ? rest[i - 1] : topN[topN.length - 1] || null));
    }
    if (setAside.length) {
      drawSectionHeader('SET ASIDE', INK_MUTE, 'Set aside for the screenable reasons noted below.');
