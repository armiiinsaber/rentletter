...
export async function buildLandlordReportPdf({ payload }) {
  ...
  blocks.forEach((b, i) => {
    ...
    if (b.fit) {
      ...
      tracked(b.word, wordX, 8, inter, b.word === 'VERIFIED' ? GREEN : INK_MUTE);
    ...
    y += LH(10.5) - SMALL;
    if (b.confirmed) { wrapWidth(b.confirmed, inter, 9.5, W - 14).forEach((ln, k) => { if (k === 0) tick(M, y + 1); text(ln, M + 14, 9.5, inter, GREEN); y -= SMALL; }); }
