...
export async function buildLandlordReportPdf({ profile, listing, active = [], setAside = [] }) {
  ...
  page.drawText('RANKED APPLICANTS', { x: MARGIN + 32, y: y + 8, size: 9, font: helvBold, color: RED });
  y -= 26;
  ...
  const drawApplicant = (row, rankNum, highlight, setAsideReasonText) => {
    ...
    if (rankNum != null) {
      page.drawText(`${rankNum}`, { x: MARGIN, y, size: 18, font: helvBold, color: highlight ? RED : INK_MUTE });
    }
  ...
  if (total === 0) {
    ...
    if (topN.length) {
      drawSectionHeader('TOP MATCHES', RED, `The ${topN.length} best fit${topN.length === 1 ? '' : 's'} for your stated criteria`);
