...
export async function buildLandlordReportPdf({ profile, listing, active = [], setAside = [], fonts }) {
  ...
  if (fonts && (fonts.headingBytes || fonts.bodyRegularBytes)) {
    try {
      ...
      if (fonts.bodyRegularBytes) bodyFont = await pdfDoc.embedFont(fonts.bodyRegularBytes, { subset: true });
      bodyBold = fonts.bodyBoldBytes ? await pdfDoc.embedFont(fonts.bodyBoldBytes, { subset: true }) : bodyFont;
      if (fonts.headingBytes) headingFont = await pdfDoc.embedFont(fonts.headingBytes, { subset: true });
      headingIsScript = !!fonts.headingIsScript;
    } catch (e) {
