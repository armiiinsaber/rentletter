...
export const DEMO_PROFILE = {
  ...
  is_founder: false, subscription_status: 'active', created_at: ago(40),
};
...
const PLACEMENT = [
  ...
  { rl: 'RL-2026-4D5E-6F70', listing: 'demo-carlaw', linkId: 'demo-link-4', reviewedAt: ago(5), decision: 'set_aside', reason: 'income_below_threshold' },
...
export function buildDemoApplicants() {
  ...
  for (const p of PLACEMENT) {
    ...
    (out[p.listing] = out[p.listing] || []).push({
      linkId: p.linkId, decisionStatus: p.decision || 'ranked', decisionPriority: p.finalist ? 'finalist' : null, decisionNotes: '', decisionReasonCode: p.reason || null,
      decisionChangedAt: p.decision ? ago(4) : null, addedVia: p.referred ? 'referral' : 'invite', reviewedAt: p.reviewedAt, reviewTracking: true,
...
export function referredApplicationForAssign() {
  const app = upgraded({
    ...
    scorecard: { incomeStability: { score: 4.8, note: '4 years at employer' }, rentAffordability: { score: 4.4, note: '35% of monthly income' }, rentalHistory: { score: 4.5, note: '3 years with reference' }, longTermIntent: { score: 4, note: 'Closer to work' }, disclosures: { score: 5, note: 'No items' }, overall: 4.5 },
  });
