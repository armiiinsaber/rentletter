...
//   confirm → the realtor's confirmations (verified state); sent → included in a sent report (sent state)
...
const PLACEMENT = [
  ...
  { rl: 'RL-2026-5E6F-7081', listing: 'demo-carlaw', linkId: 'demo-link-5', reviewedAt: ago(2), sent: true },
  { rl: 'RL-2026-6F70-8192', listing: 'demo-carlaw', linkId: 'demo-link-6', reviewedAt: null },
  { rl: 'RL-2026-7081-92A3', listing: 'demo-harbour', linkId: 'demo-link-7', reviewedAt: ago(1), docs: 'checked' },
...
export function buildDemoApplicants() {
  ...
  for (const p of PLACEMENT) {
    ...
    (out[p.listing] = out[p.listing] || []).push({
      ...
      confirmations: Object.fromEntries((p.confirm || []).map((k) => [k, { at: ago(2, 3), by: 'You' }])), lastSentAt: p.sent ? ago(1, 2) : null,
