...
import { isActive, isWithdrawn } from './listingApplicantsVocabulary';
...
const byScore = compareFit; // scoreExact descending, no Fit last, earlier applicant first on a tie
...
const RULES = [
  ...
  function topUnverified({ applicantsByListing, listings }) {
    ...
    for (const l of listings || []) {
      const active = (applicantsByListing?.[l.id] || []).filter(isActive).sort(byScore);
      const top = active[0];
  ...
  function readyToPresent({ applicantsByListing, listings, scope }) {
    const out = [];
    for (const l of listings || []) {
      const apps = (applicantsByListing?.[l.id] || []).filter((a) => !isWithdrawn(a));
  ...
  function referrals({ referralsSent, referralsInbox, now, listings }) {
    ...
    for (const r of referralsInbox || []) {
      ...
      out.push({ id: `refin:${r.id}`, urgency: 3, kind: 'referral', listingId: null,
        ...
        detail: 'Assign them to a listing to rank them against that unit.', action: { label: 'Assign', type: 'panel', target: 'referrals' } });
    }
    return out;
  },
