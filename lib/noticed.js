...
// The daily briefing for the dashboard home: what arrived, what's waiting, what's next.
// Honest when quiet. Deterministic.
export function buildBriefing({ listings, applicantsByListing, notifications, referralsInbox, notices, firstName, now }) {
  const t = now ? new Date(now) : new Date();
  const h = t.getHours(); const greet = h < 12 ? 'Good morning' : h < 18 ? 'Good afternoon' : 'Good evening';
  const lines = [];
  const unreadNew = (notifications || []).filter((n) => n.type === 'new' && n.unread);
  const unreadWd = (notifications || []).filter((n) => n.type === 'withdrawn' && n.unread);
  const unreadDocs = (notifications || []).filter((n) => n.type === 'docs' && n.unread);
  if (unreadNew.length) { const names = [...new Set(unreadNew.map((n) => n.listingName).filter(Boolean))]; lines.push(`${unreadNew.length} new application${unreadNew.length === 1 ? '' : 's'} since you last looked${names.length ? ` — ${names.slice(0, 2).join(' and ')}${names.length > 2 ? ` and ${names.length - 2} more` : ''}` : ''}.`); }
  if (unreadDocs.length) lines.push(`${unreadDocs.length} applicant${unreadDocs.length === 1 ? '' : 's'} sent the documents you asked for.`);
  if (unreadWd.length) lines.push(`${unreadWd.length} withdrew.`);
  const inbox = (referralsInbox || []).filter((r) => r.status === 'approved' && !r.assignedListingId && r.applicant);
  if (inbox.length) lines.push(`${inbox.length} applicant${inbox.length === 1 ? '' : 's'} referred to you, waiting for a listing.`);
  // what's waiting on them — from the notices (already ranked), excluding the ones echoed above
  const waiting = (notices || []).filter((c) => ['verify', 'reverify', 'stalled', 'present'].includes(c.kind));
  const total = (listings || []).reduce((s, l) => s + (applicantsByListing?.[l.id] || []).filter((a) => norm(a.decisionStatus) !== 'withdrawn').length, 0);
  // Two deliberate lines: the greeting + name as one unit, then the framing sentence.
  // (`headline` keeps the joined form for anything that wants a single string.)
  let framing;
  if (!listings?.length) framing = 'Your first listing is one link away.';
  else if (!lines.length && !waiting.length) framing = `Nothing new — ${total === 0 ? 'no applications yet' : `${total} applicant${total === 1 ? '' : 's'} across ${listings.length} listing${listings.length === 1 ? '' : 's'}, all where you left them`}.`;
  else framing = 'Here’s what today needs.';
  const greeting = firstName ? { word: `${greet},`, name: `${firstName}.` } : { word: `${greet}.`, name: '' };
  const headline = `${greeting.word}${greeting.name ? ` ${greeting.name}` : ''} ${framing}`;
  return { headline, greeting, framing, arrived: lines, waiting: waiting.map((c) => c.title), quiet: !lines.length && !waiting.length };
}
