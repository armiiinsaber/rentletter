...
export function buildBriefing({ listings, applicantsByListing, notifications, referralsInbox, notices, firstName, now }) {
  ...
  let headline;
  if (!listings?.length) headline = `${greet}${firstName ? `, ${firstName}` : ''}. Your first listing is one link away.`;
  else if (!lines.length && !waiting.length) headline = `${greet}${firstName ? `, ${firstName}` : ''}. Nothing new — ${total === 0 ? 'no applications yet' : `${total} applicant${total === 1 ? '' : 's'} across ${listings.length} listing${listings.length === 1 ? '' : 's'}, all where you left them`}.`;
  else headline = `${greet}${firstName ? `, ${firstName}` : ''}. Here’s what today needs.`;
  return { headline, arrived: lines, waiting: waiting.map((c) => c.title), quiet: !lines.length && !waiting.length };
}
