...
export async function selectExpired(admin, cutoff, { limit = BATCH } = {}) {
  ...
  if (listingIds.length) {
    // The state is what listingOpen reads; status rides along for the fallback, and the same
    // rows are read without the state where that column is absent.
    const q = (cols) => admin.from('listings').select(cols).in('id', listingIds);
    let found = await q('id, status, closed_at, state');
    if (found.error) found = await q('id, status, closed_at');
    const { data: ls, error: lErr } = found;
