...
// per invite token, 30 an hour per IP. Other public surfaces pass their own scope and limits
// (the short link: scope 'short', 60 an hour per IP). The eleventh (or thirty first) gets a 429 with a plain
...
export async function checkSubmitLimits(kv, { token, ip, now = Date.now(), scope = 'apply', tokenLimit = LIMITS.token, ipLimit = LIMITS.ip } = {}) {
  const checks = [];
  if (token) checks.push({ kind: 'token', key: `rl:${clean(scope)}:token:${clean(token)}:${bucket(now)}`, limit: tokenLimit });
  if (ip) checks.push({ kind: 'ip', key: `rl:${clean(scope)}:ip:${clean(ip)}:${bucket(now)}`, limit: ipLimit });
