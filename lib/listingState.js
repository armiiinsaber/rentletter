...
import { LEGACY_LISTING_STATUS, LEGACY_LISTING_STATUSES, isLegacyRented, DECISION_STATUS } from './application-state.js';
...
export const listingStatus = (l) => (l && LISTING_STATUSES.includes(l.status) ? l.status : LEGACY_LISTING_STATUS.ACTIVE);
export const listingOpen = (l) => listingStatus(l) === LEGACY_LISTING_STATUS.ACTIVE;
...
export function notSelectedRecipients(junctions, chosenLinkId) {
  ...
  for (const j of junctions || []) {
    ...
    if ((j.decision_status || j.decisionStatus || DECISION_STATUS.NONE) === DECISION_STATUS.REJECT) continue;
