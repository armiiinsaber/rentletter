...
// The values live in lib/application-state.js (the single source of truth for every status).
import { LEGACY_LISTING_STATUS, LEGACY_LISTING_STATUSES, isLegacyRented, applicantStanding, listingStanding } from './application-state.js';
export const LISTING_STATUSES = LEGACY_LISTING_STATUSES;
...
export function statusPatch(status, { now = new Date(), rentedLinkId = null } = {}) {
  ...
  if (status === LEGACY_LISTING_STATUS.ACTIVE) return { status: LEGACY_LISTING_STATUS.ACTIVE, closed_at: null, rented_link_id: null };
  return { status, closed_at: new Date(now).toISOString(), rented_link_id: isLegacyRented(status) ? (rentedLinkId || null) : null };
}

// The listing's status as the app reads it: absent columns read as active.
// Both answer from lib/application-state.js listingStanding: the state column when the row
// carries one, the status column only when it does not. Open means live.
export const listingStatus = (l) => listingStanding(l).legacyStatus;
export const listingOpen = (l) => listingStanding(l).open;
...
export function notSelectedRecipients(junctions, chosenLinkId) {
  ...
  for (const j of junctions || []) {
    ...
    if (!applicantStanding(j).active) continue; // withdrawn or set aside
