...
// how an event reads and links. Every event is recorded by a route on the server.
//
...
// Every realtor action is written by a route on the server (lib/realtorWrites.js), which records
// its event through the service role with profile_id from the session. No client reports events.
...
export function eventTitle(e) {
  ...
  switch (e && e.type) {
    ...
    case 'report_opened': return `${p.sentToName || 'The landlord'} opened the report for ${where(p)}`;
    case 'landlord_answered': return `${p.sentToName || 'The landlord'} answered for ${where(p)}: ${p.answer === 'meet' ? 'wants to meet' : 'not for me'}, ${p.applicantName || `rank ${p.rank}`}`;
    ...
    case 'pipeline_invited': return `You invited ${who(p)} to apply to ${where(p)}`;
    case 'pipeline_renewal_sent': return `${who(p)} was asked to renew keep me in mind`;
    case 'pipeline_removed': return `You removed ${who(p)} from Pipeline`;
    case 'reference_requested': return `You asked ${who(p)}'s previous landlord six questions`;
    case 'reference_answered': return `${who(p)}'s previous landlord answered`;
