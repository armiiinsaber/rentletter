...
export async function purgeStoredDocument(admin, { id, linkId, deletedBy = 'tenant removed' }) {
  ...
  const { error: upErr } = await admin.from('applicant_documents').update({ deleted_at: new Date().toISOString(), deleted_by: String(deletedBy || 'tenant removed').slice(0, 80) }).in('id', live.map((r) => r.id));
