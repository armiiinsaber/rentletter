...
export async function analyzeOneFile(deps, body) {
  ...
  if (store) {
    try { await store({ profileId: userId, listingId: own.listing.id, linkId, applicationId: own.junction.application_id, applicantName: own.application.full_name || null, uploadedBy: 'realtor', files: [{ mime, data, name }], replace: Object.keys(staging.items).length === 0 }); }
