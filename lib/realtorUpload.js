...
import { ALLOWED_DOC_MIME, MAX_DOCS, screenableFacts } from './applicantAnalysis.js';
...
export async function finalizeAnalysis(deps, body) {
  ...
  const run = { ...buildCombinedRun(items, own.application.full_name || '', screenableFacts(own.application, own.listing)), analyzedAt: new Date(now).toISOString(), source: 'realtor' };
