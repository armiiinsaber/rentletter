...
// ARCHIVE the active report into archived[], clearing active. The entry keeps its ai_insight key
// for older rows; nothing writes a value into it any more (the insight was cut).
export function withArchivedActive(raw, archivedInsight) {
  const n = normalizeDocV(raw);
  if (!n.active) return { active: null, archived: n.archived };
  const entry = {
    id: makeArchiveId(n.active), report: n.active, ai_insight: archivedInsight || null,
