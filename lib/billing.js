...
export async function createCheckout({ profile, email, interval }) {
  ...
  const s = await stripe('POST', '/checkout/sessions', {
    ...
    success_url: `${site()}/dashboard?checkout=success`, cancel_url: `${site()}/dashboard?checkout=canceled`,
  });
...
export async function createPortal({ profile }) {
  ...
  const s = await stripe('POST', '/billing_portal/sessions', { customer: profile.stripe_customer_id, return_url: `${site()}/dashboard` });
  return { url: s.url };
}
