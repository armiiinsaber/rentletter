...
import { listPeople } from './pipeline.js';
...
async function loadSignalsInner({ supabase, admin, user, listings }) {
  ...
  // People (lib/pipeline.js): three reads after the junction read, since applied needs the
  // active listings' applicants. Empty until db/pipeline.sql and db/listing-status.sql have run.
  const people = await safe(listPeople({ profileId: user.id, listings: all, admin, applicantsByListing }), []);
