...
async function loadSignalsInner({ supabase, admin, user, listings }) {
  ...
  const [latestEvent, notif, inbox, sent, byListing, peopleSrc] = await Promise.all([
    ...
    safe(readPeopleSource({ profileId: user.id, admin }), null), // People: three reads, beside the junction read
  ]);
  const applicantsByListing = {};
  ls.forEach((l) => { applicantsByListing[l.id] = byListing[l.id] || []; });
  // People (lib/pipeline.js): combined here, since applied needs the active listings' applicants.
