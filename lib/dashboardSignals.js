...
// server load (pages/dashboard.js, so the first paint has it) and GET /api/assistant/signals
...
import { referralsEnabled } from './features.js';
import { readPeopleSource, peopleRows } from './pipeline.js';
import { traceEnabled, startTrace, endTrace, tracedClient } from './queryTrace.js';
import { LEGACY_LISTING_STATUS } from './application-state.js';
...
async function loadSignalsInner({ supabase, admin, user, listings }) {
  ...
  const [latestEvent, notif, inbox, sent, byListing, peopleSrc] = await Promise.all([
    ...
    referralsEnabled() ? safe(inboxFor(user), []) : Promise.resolve([]), // lib/features.js: the two KV reads are skipped while referrals are paused
    referralsEnabled() ? safe(listFromRealtor(user.id), []) : Promise.resolve([]),
    safe(fetchApplicantsForListings(admin, ls), {}),
    safe(readPeopleSource({ profileId: user.id, admin }), null), // Pipeline: three reads, beside the junction read
  ]);
  const applicantsByListing = {};
  ls.forEach((l) => { applicantsByListing[l.id] = byListing[l.id] || []; });
  // Pipeline (lib/pipeline.js): combined here, since applied needs the active listings' applicants.
  // Empty until db/pipeline.sql and db/listing-status.sql have run.
  const people = peopleSrc ? peopleRows({ ...peopleSrc, listings: all, applicantsByListing }) : [];
  ...
  return JSON.parse(JSON.stringify({
    listings: all.map((l) => ({ id: l.id, name: l.name, address: l.address, unit: l.unit ?? null, monthly_rent: l.monthly_rent, bedrooms: l.bedrooms ?? null, created_at: l.created_at || null, landlord_email: l.landlord_email, landlord_name: l.landlord_name, status: l.status || LEGACY_LISTING_STATUS.ACTIVE, closed_at: l.closed_at || null, rented_link_id: l.rented_link_id || null })),
    applicantsByListing, people, notifications: notif?.items || [], referralsInbox: inbox || [], referralsSent: Object.values(byLink), latestEventAt: latestEvent || null, loaded: true,
  }));
}
