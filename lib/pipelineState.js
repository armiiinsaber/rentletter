// lib/pipelineState.js  PURE, shared by the browser and the server. People: the pipeline.
//
...
//   peopleRows(...)             the list: one row per consent, Fit against every active listing.
//   inviteEmail / renewalEmail  the two messages, in the realtor's voice.
//   pipelineFitItems(...)       the "People who fit" item per new active listing (lib/actions.js).
import { computeFit } from './fitScore.js';
import { listingOpen, CONSENT_DAYS } from './listingState.js';

const DAY = 86400000;
export const PREFILL_TTL = 14 * DAY / 1000; // seconds: the invite's prefill link lives 14 days
...
// The list. consents: rows for this realtor with status consented and expires_at ahead (the
// caller filtered by profile_id; this filters status and expiry again). applications: the rows
...
export function peopleRows({ consents, applications, junctions, listings, applicantsByListing, now = new Date() } = {}) {
  ...
  for (const c of consents || []) {
    if (!c || c.status !== 'consented') continue;
...
// "People who fit": one item per active listing created after the consent rows existed (a new
...
export function pipelineFitItems({ listings, people } = {}) {
  ...
  for (const l of listings || []) {
    ...
    const n = (people || []).filter((p) => !p.applied && (p.fits || []).some((f) => String(f.listingId) === String(l.id) && f.score != null && f.score >= FIT_THRESHOLD && !f.applied)).length;
    if (!n) continue;
    ...
    out.push({ key: `pipeline_fit:${l.id}`, kind: 'pipeline_fit', listingId: l.id, linkId: null, title: 'People who fit', detail: `${n} ${n === 1 ? 'person' : 'people'} at ${FIT_THRESHOLD.toFixed(1)} or above · ${listingLabel(l)}`, verb: 'Open', panel: 'people', name: null, listingName: listingLabel(l), reason: `${n} ${n === 1 ? 'person' : 'people'} at ${FIT_THRESHOLD.toFixed(1)} or above`, since: l.created_at || null, signature: `pipeline_fit:${l.id}:${n}` });
  }
  return out;
}
