...
export default async function handler(req, res) {
  ...
  const { email, fullName, letter, resume, applicationNumber, ownerToken } = req.body;

  // A letter (legacy paid flow) OR an application number (current apply flow) is required.
  if (!email || (!letter && !resume && !applicationNumber)) {
    return res.status(400).json({ error: 'Missing email and content' });
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  ...
  try {
    // ── CURRENT PATH — application confirmation, NO attachments. Any send without a
    // cover letter is a plain confirmation: the legacy rent-letter PDF (which rendered
    // with an empty letter page) and the tenant-résumé attachment are gone. A `resume`
    // field from a stale client is deliberately ignored.
    if (!(letter || '').trim()) {
    ...
    // ── LEGACY PATH — the paid cover-letter product (pages/index.js flow) still emails
    // the letter + templated résumé as PDF/DOCX. Unchanged.
    const safeLetter = letter || '';
    const safeResume = resume || '';
    const docxBuffer = await buildDocx(safeLetter, safeResume, fullName || 'Applicant');
    const pdfBuffer = await buildPdf(safeLetter, safeResume, fullName || 'Applicant');
    ...
    const result = await resend.emails.send({
      from: 'Rentletter <hello@rentletter.ca>',
      to: email,
      subject: safeLetter ? 'Your letter is ready' : 'Your Rentletter application',
      html: `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
