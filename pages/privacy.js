...
**2.3 Documents: held for the realtor's review, then deleted.** A realtor may upload supporting documents an applicant has provided (such as a pay stub, employment letter, or identification), or an applicant may upload them through a secure link, so that the Service can read and verify the information against the application. The file is analyzed to extract relevant, factual information (for example, stated income and employer name). **The original is then held in private storage for 14 days, or until the realtor deletes it, whichever comes first.** During that time only the realtor handling the listing can view it, through a short lived link that expires within one minute, and every view and deletion is logged. Documents are never written to logs, never shared with landlords or other realtors, and are removed from storage when they expire or are deleted. The extracted factual results and verification outcomes are retained separately.
...
- **Uploaded documents:** held for up to **fourteen (14) days** after upload, or until the realtor deletes them, whichever comes first (see Section 2.3). Extracted factual results are retained subject to the same retention period as the related application.
