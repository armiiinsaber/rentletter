...
// listing opens its detail view (/listing/[id]). Stage 1: no KV workspace.
...
export async function getServerSideProps(ctx) {
  ...
  if (!user) {
    return { redirect: { destination: '/signin?next=/dashboard', permanent: false } };
  }
  ...
  if (profileRead.failed) return sessionRedirect('/dashboard');
